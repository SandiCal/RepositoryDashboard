...
# Remove future dates
datasets$pub_date <- mdy(datasets$pub_date) # if this line fails, dates may already be formatted as dates, in which case comment out this line
...
datasets <- mutate(datasets,
  ...
  # Reorder columns for easier viewing.
      select(FY, doi, num_bytes, kb, mb, gb, num_files, everything())
```
...
# Remove future dates 
datafiles$pub_date <- mdy(datafiles$pub_date) # if this line fails, dates may already be formatted as dates, in which case comment out this line
...
# Reorder columns for easier viewing.       
  select(FY, doi, num_bytes, kb, mb, gb, file_format, everything())
...
downloads <- fromJSON(file = "downloads.json")
downloads <- downloads %>%                    
  gather_array %>%          
  spread_values(            
    doi = jstring("doi"), 
...
ggplot(sumtime, aes(x= FY, y = Datasets)) +
  geom_col(fill="darkcyan") +
  geom_text(aes(label = Datasets), vjust = -0.5) +
  theme_classic()+
  theme+
  labs(title = "Datasets Published per FY")+
  # create x axis labels for every 1 year
...
ggplot(sumtime, aes(x= FY, y = GB)) +
  geom_col(fill = "chocolate") +
  geom_text(aes(label = round(GB)), vjust = -0.5) +
  theme_classic()+
  theme+
  labs(title = "GBs Published per FY")+
  # create x axis labels for every 1 year
  scale_x_continuous(breaks = breaks_width(1))
...
ggplot(sumtime, aes(x= FY, y = Files)) +
  geom_col(fill = "darkolivegreen4") +
  geom_text(aes(label = Files), vjust = -0.5) +
  theme_classic()+
  theme+
  labs(title = "Files Published per FY")+
  # create x axis labels for every 1 year
  scale_x_continuous(breaks = breaks_width(1))
...
# plot
ggplot(sumdl, aes(x=FY, y=total)) +
  geom_segment( aes(x=FY, xend=FY, y=0, yend=total), color="grey", size=1.5) +
  ...
  xlab("") +
  ylab("Downloads") +
  labs(title = "Downloads per Year") +
  
  # create x axis labels for every 1 year
  scale_x_continuous(breaks = breaks_width(1))+
  # prevent scientific notation in y axis labels and make room for data labels
...
### Data as of 6/30/2025. To zoom in on individual elements, make your window narrower until it displays one chart at a time.
...
knitr::kable(sizestats, digits = 0, scientific = FALSE)       
```
...
knitr::kable(filestats, digits = 0, scientific = FALSE)
```
...
### Data as of 6/30/2025. % indicates percentiles (i.e. 95% indicates the bottom 95% of datasets, excluding the top 5% as outliers).
...
ggplot(sumsub, aes(x= subject, y = Datasets, fill = subject)) +
  ...
  labs(title = "Datasets Published per Subject",
       x = "Subject",
       y = "Datasets"
       )+
...
ggplot(sumsub, aes(x= subject, y = GB, fill = subject)) +
  geom_col() +
  scale_fill_manual(values = mycolsmed) +
  geom_text(aes(label = round(GB)), vjust = -0.2) +
  coord_cartesian(clip = 'off') +
  guides(x = guide_axis(angle = 45)) +
  ...
  theme +
  theme(legend.position="none") +
  labs(title = "GBs Published per Subject",
       x = "Subject",
       y = "GB"
       )+
  # make room for data labels
...
ggplot(sumsub, aes(x= subject, y = Files, fill = subject)) +
  ...
  guides(x = guide_axis(angle = 45)) +
  theme(legend.position="none") +
  labs(title = "Files Published per Subject",
       x = "Subject",
       y = "Files (count)"
       )+
  # make room for data labels
...
cumulative <- as_tibble(datasets |>
  ...
  group_by(FY) |> 
  count(subject)) |> 
  complete(FY, subject, fill = list(n = 0)) |> ## represent all subjects in all years even if the number of pubs is 0
  group_by(subject) |>
...
ggplot(cumulative,
       aes(x= FY, y = c_subtotal, fill = subject)) +
  ...
  labs(title = "Cumulative Dataset Count by Subject",
       x = "FY",
       y = "Total Datasets")+
  # create x axis labels for every 1 year
  scale_x_continuous(breaks = breaks_width(1))
...
select(FY, doi, num_bytes, kb, mb, gb, file_format, file_type, file_subtype, file_bucket, everything())
```
...
knitr::kable(datafiles |> 
  ...
  digits = 0, col.names = c("File Format", "Count"), 
...
knitr::kable(datafiles |> 
  ...
  summarise(GB = sum(GB)) |> 
  arrange(desc(GB)),
  digits = 2, col.names = c("File Format", "GB"), 
...
knitr::kable(sumtype, digits = 2, col.names = c("File Type", "Count", "Total GB", "Ave GB"), format.args = list(big.mark = ",",
  scientific = FALSE))  
...
knitr::kable(sumbucket, digits = 2, col.names = c("File Bucket", "Count", "Total GB", "Ave GB"), format.args = list(big.mark = ",",
  scientific = FALSE))  
